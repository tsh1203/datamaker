...
#ifdef _WIN32
  #include <conio.h>   // _getche — Windows only, no conflict with std::byte
#endif
using namespace std;
...
// ---------- 询问是否重新生成数据（RE 时调用）----------
// 返回 true 表示用户选择重新生成
bool askRegen(int caseNo) {
    cerr << "\033[33m[ASK]  \033[0mRE on case #" << caseNo
         << "  regenerate all data? [y/N] ";
    char ch = 0;
#ifdef _WIN32
    ch = (char)_getche();
#else
    FILE* tty = fopen("/dev/tty", "r");
    if (tty) { ch = (char)fgetc(tty); fclose(tty); }
#endif
    cerr << "\n";
    return (ch == 'y' || ch == 'Y');
}
...
signed main(int argc, char* argv[]) {
    ...
    if (!fin) { log_err("cannot open data.txt"); return 1; }
    string name, line;
    ...
    fin.close();
    if (name.empty()) { log_err("problem line not found in data.txt"); return 1; }
...
restart:
    cnt = 0; failCnt = 0; totalTime = 0; failList.clear();
    ...
    while (true) {
        ...
        if (!stdOk) {
            log_err("std RE on case #" + to_string(cnt));
            if (askRegen(cnt)) {
                log_info("regenerating ...");
                if (!run("data.exe")) { log_err("data.exe failed"); return 1; }
                log_ok("data regenerated, restarting check");
                goto restart;
            }
